...
define config.say_attribute_transition = Dissolve(0.15)

#SMOOTH INITIAL MAIN SCREEN
define config.end_splash_transition = Dissolve(0.2)
...
define config.name = _("Midnight Witch")
...
define build.name = "MidnightWitch"
...
define config.save_directory = "MIDNIGHTWITCH-1644712243"
...
init python:
    ...
    build.classify("game/audio/music/**.ogg", "audio")
    build.classify("game/audio/music/**.mp3", "audio")
    build.classify("game/audio/sound/**.ogg", "audio")
    build.classify("game/audio/sound/**.wav", "audio")
...
## Enable language selection in Preferences menu
define config.languages = ["english", "polish"]

## Optional: Set Polish as default language (comment out to keep English default)
# define config.language = "polish"
